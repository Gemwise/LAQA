...
public class CacheThread extends Thread{
	...
	public CacheThread(String folder,int startIndex, int getEndIndex, int threadID) {
		dirName = folder;
		flag = true;
		index = startIndex;
...
synchronized void readTile(int id) {
	...
	public void run() {
        ...
        System.out.println(threName + ": file table preparation done, time used: "+(tablePrepTime - startPrepTime)/1000 + "s");
		...
		if (flag) {
			...
			while (readTiles.size () != files.size ()) {
...
//            int crf = Integer.parseInt(fileName.substring(fileName.indexOf("crf")+3,fileName.indexOf(".265")));
