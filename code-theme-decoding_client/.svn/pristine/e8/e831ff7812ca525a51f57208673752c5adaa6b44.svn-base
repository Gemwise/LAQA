...
public class Utils {
    ...
    public static int getVideoID(String indexPos, int tileID, int quality) {
        String pose = indexPos+","+tileID+","+quality; //0,0,0,15
    ...
    public static int getQuality(int videoID){
        String pose  = Utils.videoID2pose.get(videoID);
        return Integer.parseInt(pose.split(",")[3]);
    }
    ...
    public static ArrayList<Integer>  getCurVisibleTiles() {
        ...
        ArrayList<Integer> al = new ArrayList<>(); //存放 videoID

        String coor = "("+(int) Utils.coorX+","+(int) Utils.coorY+","+0+")";
        //System.out.println(coor);
        //(-90,-175,0)
        //2.0,3.0,0.0,1.0
        ArrayList<Integer> tiles = Utils.tileTable.get(coor); //都是靠 FoV 值去查询 txt文档，获取对应的tiles
        //System.out.println(tiles)

        //检查可用质量等级
        //eg:avaQuality = {15, 19, 23, 27, 31, 35}
        //getVideoID
        //0 0,0,0,15
        //1 0,0,0,19
        //2 0,0,0,23
        ...
        //22 0,0,3,31
        //23 0,0,3,35
        // list available tiles 用于存放当前可用的质量等级
        ...
        for (int i=0;i<Config.qualityLevel;i++) {
            ...
            for (int tileId : tiles) {
                ...
                //如果未接收到，则不将该质量等级加入 avaQuality 列表。
                if(!NetworkBufferPool.videoReceived.contains(videoID)){
                    videoID = -1;
                }
                if (videoID == -1) break;
                //只要当前fov内有 一个tile未收到，就直接退出。
            }
            if (videoID != -1)
                avaQuality.add(Config.avaQuality[i]);
        }

        //选择合适的质量等级：
        int quality = -1;
        //如果 avaQuality 只有一个可用质量，则直接选择这个质量。
        if (avaQuality.size() == 1){
            quality = avaQuality.get(0);
        }
        else if (avaQuality.size() > 1){
            //如果有多个可用质量，代码会选择与之前显示质量最接近的那个，尽量减少质量波动带来的影响。
            int min = 100;
            for (int i = 0; i < avaQuality.size(); i++){
                ...
                int diff = quality; //由于逻辑错误，quality 被锁定为第一个值 15,后续循环中，无论 avaQuality 中有哪些值，代码都会错误地跳过更新
        ...
        }//只是我的代码中，目前的逻辑，在一个fov下，不会同时出现不同的quality的tile，都是同一种。
        ...
        for(int tileId : tiles) {
            int videoID = -1;
            videoID = Utils.getVideoID(Utils.naviPos, tileId, quality);//0 0,0,0,15
            al.add(videoID);
        }
