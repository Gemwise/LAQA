...
public class RTPNetwork extends Thread{
    ...
    public void run(){
        ...
        while(!Utils.endTransmission) {
            try {
                ...
                videoID = rtp_packet.getvideoid(); //服务器端发送的时，会携带这些信息
                ...
                if(tileLen != 0) {
                    ...
                    int payload_length = rtp_packet.getpayloadlength(); //Byte
    ...
    void analyPose(int videoID, float[] orientations){
        ...
        String indexPos = Utils.getPosFromMsg(Utils.videoID2pose.get(videoID)); // 得到 pos 信息
        float oriX = Utils.calAngle(orientations[0]);
        float oriY = Utils.calAngle(orientations[1]); //得到 FOV 信息
