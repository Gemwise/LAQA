...
# 改为进度条模式
for k in tqdm(range(config.TOTAL_TRACE_NUM), desc="Round Processing", unit=" round", leave=True):
    ...
    for policy in policies:
        ...
        # 改为进度条模式
        for t in tqdm(range(config.T), desc="Processing Data", unit="item", leave=False):
            # prediction result
