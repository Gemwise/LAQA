...
with plt.style.context(['science', 'grid']):
    x = np.arange(5)
    n4k = [quality[0], var[0], qoe[0], trans[0], 0]
    k4 = [quality[1], var[1], qoe[1], trans[1], deco]
    bar_with = 0.3
    tick_label = ['Mean Quality', 'Mean Variance',
                  'Mean QoE', 'Mean Trans Delay', 'Mean Decode Latency']
    plt.bar(x, n4k, bar_with, align='center',
            color='#96C37D', label='No Decoding Time')
    plt.bar(x+bar_with, k4, bar_with, align='center',
            color='#D8383A', label='Decoding Time')
    ...
    for i in range(5):
        ...
        plt.annotate(formatted_x, (i+bar_with,
                     k4[i]), ha='center', fontweight='bold')

    plt.legend(loc='best')
    plt.tight_layout()
    os.chdir("E:/lyq/02-myWork/01-ICCC/ICCC_CODES/simulation/draw/sim_part/")
    plt.savefig("nodecode"+'.pdf', format='pdf', dpi=300)
